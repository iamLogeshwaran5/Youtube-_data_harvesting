...
icpic = Image.open("youtube.png")

def main():
    st.set_page_config(
        page_title= "Youtube Data Project",
        page_icon = icpic,
        layout = "centered"
    )
...
fetch_json, fetch_text, upload_data, view_channel, migrate_sql, channel_anlysis = st.tabs([" Fetch Json Data ", " Fetch Text Data ", " Upload To MongoDB ", " View Channel Tables",  " Migrate To SQL ", " Channel Analysis "])
...
with fetch_json:
    if st.button("Fetch JSON Data"):
        try:
            ...
            st.json(data)
            st.success("Successfully fetch Json")
        except:
...
with fetch_text:
    if st.button("Fetch Text Data"):
        try:
            data = blind_data(api_key,channel_id)
            text_output = format_data_astext(data)
            st.text(text_output)
...
with upload_data:
    ...
    with migrate_sql:
        ...
        if migrate_SQL:
            with st.spinner("Uploading data to MYSQL..."):
                try:
                ...
                try:
                    ...
                    st.success("Data uploaded successfully to MySQL!")
                except Exception as e:
                    st.error(f"An error occurred while uploading data to MySQL: {str(e)}")
